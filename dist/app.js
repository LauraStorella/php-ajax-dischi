...
$(document).ready(function () {
  ...
  getCds(''); // Filtro per autore tramite select e stampa dei relativi cd
...
function getCds(filterAuthor) {
  // Reset container cd
  $('.cds-container').html(''); // chiamata ajax per recuperare dati dischi
...
function printCds(arrayCds) {
  ...
  var template = Handlebars.compile(source); // Creo ciclo For per stampare tutti gli album in array

  for (var i = 0; i < arrayCds.length; i++) {
    var singleCd = arrayCds[i];
    console.log(singleCd); // Inserisco info album nei placeholder di handlebars template
    // var context = {
